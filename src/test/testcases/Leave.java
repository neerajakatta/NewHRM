...
public class Leave extends UserLibrary
{
	...
	//=========================================
	public static void TC151_searchLeaveDetails()
	{

		//Step 1: Login to Team Member
	...
	//=========================================
		public static void TC150_applyLeave_VerifyMyLeave()
		{
			Boolean retValue;
			//Step 1: Login to Team Member
			Login(URL, UserName , Password);
			
			//Step 2: Navigate to Apply Leavepage
			NavigateApplyeave();
			
			//Step 3: Select Leave Type Drop Down
			retValue = SelectOPtionByText("//select[@id='applyleave_txtLeaveType']",LeaveType);
			
			if (retValue)
				LogEventWithScreeshot("info", "Annual Leave Selected");
			else
				LogEventWithScreeshot("fail", "Annual Leave not Selected");
						
			//Step 4: Select from date
			SelectDateRange("//input[@id='applyleave_txtFromDate']","//input[@id=applyleave_txtToDate]");
			
			//Step 5: Click Apply
			//ClickElement("//input[@id=applyBtn]");
			
			if (retValue)
				LogEventWithScreeshot("info", "Apply butoon selected");
			else
				LogEventWithScreeshot("fail", "Apply butoon not selected");
	...
	public static void TC30_SearchLeaveStatus()
	{
		boolean status;
		//Step 1: Login HRM as a team member
		Login(URL, UserName , Password);
		
		//Step 2: Navigate to MyLeave page
		NavigateMyLeave();
		
		//Step 3: Select Leave status and click Search button
		status = SelectLeaveStatus(LeaveStatus);
		if(status)
		{
			LogEventWithScreeshot("pass", "Leave status selected");
		}
		else
		{
			LogEventWithScreeshot("fail", "Leave status not selected");
		}
		//Click search
		status = ClickElement("//*[@id='btnSearch']");
		if(status)
		{
			LogEventWithScreeshot("pass", "Search button clicked");
		}
		else
		{
			LogEventWithScreeshot("fail", "Search button clicked");
		}
		
		//Step 4: Verify Leave status in My Leave table
